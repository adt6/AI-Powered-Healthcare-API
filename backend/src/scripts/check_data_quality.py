...
def check_null_values(db: Session, model_class, table_name: str):
    ...
    for column in columns:
        ...
        # Count nulls
        null_count = db.query(func.count()).filter(column.is_(None)).scalar()
...
def check_foreign_key_integrity(db: Session):
    ...
    encounters_with_practitioners = (
        db.query(EncounterV2).filter(EncounterV2.practitioner_id.isnot(None)).count()
    )
    ...
    if total_encounters > 0:
        ...
        print(
            f"ℹ️  {encounters_with_practitioners}/{total_encounters} encounters have practitioners ({practitioner_percent:.1f}%)"
        )

    # Check encounters -> organizations
    encounters_with_orgs = (
        db.query(EncounterV2).filter(EncounterV2.organization_id.isnot(None)).count()
    )

    if total_encounters > 0:
        org_percent = (encounters_with_orgs / total_encounters) * 100
        print(
            f"ℹ️  {encounters_with_orgs}/{total_encounters} encounters have organizations ({org_percent:.1f}%)"
        )
    ...
    for cond in conditions:
        if cond.patient_id:
            patient = (
                db.query(PatientV2).filter(PatientV2.id == cond.patient_id).first()
            )
            if not patient:
...
def check_data_completeness(db: Session):
    ...
    for table_name, model_class in MODELS.items():
        ...
        print(
            f"{status} {table_name.capitalize():<20} Rows: {total_rows:<10} Required columns missing: {missing_required}"
        )
...
def identify_redundant_columns(db: Session):
    ...
    for table_name, model_class in MODELS.items():
        ...
        if redundant_cols:
            ...
            print(f"\n{table_name.upper()}:")
            for col_name, null_pct in sorted(
                redundant_cols, key=lambda x: x[1], reverse=True
            ):
